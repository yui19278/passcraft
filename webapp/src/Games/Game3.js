...
const Game3 = () => {
  ...
  const handleSubmit = () => {
    ...
    setHistory((prev) => [
      ...prev,
      {
        ...
        correctAnswers: Array.isArray(currentQuestion.answers)
          ? currentQuestion.answers
          : null,
      },
    ]);
    ...
    return (
      <div className="game-container fade-in">
        ...
        <p className="question-text">
          SNSに投稿した写真から色んな情報が分かるって知ってた？
          <br />
          どんな情報が読み取れるか試してみよう！
        </p>
        <hr />
        <p>
          あなたのSNSに，ある写真がアップされました.
          <br />
          でもその写真には意外な「手がかり」が...
        </p>
        <p>
  ...
  if (step === "loading")
    return <div className="game-container">読み込み中...</div>;
  if (step === "error")
    return (
      <div className="game-container">エラー：問題が読み込めませんでした。</div>
    );
                ...
                <li key={i} style={{ marginBottom: "1em" }}>
                  ...
                  {Array.isArray(item.correctAnswers)
                    ? item.correctAnswers
                        ...
                        .includes(item.userAnswer.trim().toLowerCase())
                      ? " ✅ 正解"
                      : " ❌ 不正解"
      ...
      {step === "summary2" && (
        <div className="fade-in">
          ...
          <div className="summary-points">
            <ul>
              <li>
                位置情報を消したつもりでも，写真1枚で居場所が特定される可能性がある．
              </li>
              <li>知らぬ間に個人情報が外部に漏れてしまうリスクがある．</li>
              <li>
                投稿する前に，写り込んでいる情報を見直すことがとても重要．
              </li>
            </ul>
          </div>
          <p>
